...
steampipe_ingest.py
-------------------
Queries live AWS resource data via Steampipe (or sample JSON for testing)
and syncs it into your cloud_security PostgreSQL database.

How it works:
  - Connects to Steampipe's built-in Postgres (port 9193)
  - Queries aws_ec2_instance, aws_s3_bucket, aws_iam_user, etc.
  - Writes results into your cloud_security DB (port 5432)
  - This gives you a live inventory to JOIN with Prowler/Trivy findings

Usage
-----
# Sync live AWS data (requires Steampipe running + AWS credentials):
    python3 steampipe_ingest.py --sync

# Test with sample data (no AWS needed):
    python3 steampipe_ingest.py --sample

# Sync specific resource types only:
    python3 steampipe_ingest.py --sync --tables instances s3 iam
"""
...
# ── Your cloud_security DB (port 5432) ───────────────────────────────────────
SECURITY_DB = {
...
# ── Steampipe's built-in Postgres (port 9193) ─────────────────────────────────
STEAMPIPE_DB = {
    "host":    "localhost",
    "port":    9193,
    "dbname":  "steampipe",
    "user":    "steampipe",
    "password": os.getenv("STEAMPIPE_PASSWORD", ""),  # from: steampipe service status
}
...
# ═════════════════════════════════════════════════════════════════════════════
# STEAMPIPE QUERIES (run against port 9193)
# ═════════════════════════════════════════════════════════════════════════════
...
STEAMPIPE_QUERIES = {
    ...
    "iam": """
        SELECT
            ...
            user_id,
            arn                 AS resource_arn,
            mfa_active          AS mfa_enabled,
            password_enabled    AS has_console_access,
            (access_keys IS NOT NULL AND jsonb_array_length(access_keys) > 0) AS has_access_keys,
            password_last_used,
            NULL::timestamptz   AS access_key_last_used,
            create_date,
            tags,
            to_jsonb(u)         AS raw_json
        FROM aws_iam_user u
    """,
...
# ═════════════════════════════════════════════════════════════════════════════
# UPSERT FUNCTIONS (write into cloud_security DB on port 5432)
# ═════════════════════════════════════════════════════════════════════════════
...
def upsert_instances(conn, rows):
    ...
    """
    template = """(
        %(instance_id)s, %(account_id)s, %(region)s, %(resource_arn)s,
...
def upsert_s3(conn, rows):
    sql = """
        INSERT INTO aws_s3_buckets (
            bucket_name, account_id, region, resource_arn,
            public_access_blocked, versioning_enabled, logging_enabled,
            encryption_enabled, creation_date, tags, raw_json, synced_at
        ) VALUES %s
    ...
    """
    template = """(
        %(bucket_name)s, %(account_id)s, %(region)s, %(resource_arn)s,
...
def upsert_iam(conn, rows):
    sql = """
        INSERT INTO aws_iam_users (
            user_name, account_id, user_id, resource_arn,
            mfa_enabled, has_console_access, has_access_keys,
            password_last_used, access_key_last_used, create_date,
            tags, raw_json, synced_at
        ) VALUES %s
        ON CONFLICT (user_id) DO UPDATE SET
            mfa_enabled         = EXCLUDED.mfa_enabled,
            has_access_keys     = EXCLUDED.has_access_keys,
    ...
    """
    template = """(
        %(user_name)s, %(account_id)s, %(user_id)s, %(resource_arn)s,
...
def upsert_security_groups(conn, rows):
    sql = """
        INSERT INTO aws_security_groups (
            group_id, group_name, account_id, region, resource_arn,
            vpc_id, description, allows_all_ingress, allows_ssh_public,
            allows_rdp_public, tags, raw_json, synced_at
        ) VALUES %s
    ...
    """
    template = """(
        %(group_id)s, %(group_name)s, %(account_id)s, %(region)s, %(resource_arn)s,
...
def upsert_vpcs(conn, rows):
    sql = """
        INSERT INTO aws_vpcs (
            vpc_id, account_id, region, cidr_block,
            is_default, state, tags, raw_json, synced_at
        ) VALUES %s
    ...
    """
    template = """(
        %(vpc_id)s, %(account_id)s, %(region)s, %(cidr_block)s,
...
# ═════════════════════════════════════════════════════════════════════════════
# LIVE SYNC (requires Steampipe running + AWS credentials)
# ═════════════════════════════════════════════════════════════════════════════

def sync_live(tables_to_sync):
    print(f"[{ts()}] Connecting to Steampipe (port 9193)...")
    sp_conn  = psycopg2.connect(**STEAMPIPE_DB)
    sec_conn = psycopg2.connect(**SECURITY_DB)

    for table in tables_to_sync:
        query = STEAMPIPE_QUERIES.get(table)
        if not query:
            print(f"[{ts()}] Unknown table: {table}, skipping")
            continue

        print(f"[{ts()}] Querying Steampipe: {table}...")
        start = time.time()
        try:
            ...
            print(f"[{ts()}] ✓ {table}: synced {len(rows)} rows ({duration}ms)")

        except Exception as e:
            duration = int((time.time() - start) * 1000)
            log_sync(sec_conn, table, 0, duration, status="error", error=str(e))
            print(f"[{ts()}] ✗ {table} failed: {e}")
...
# ═════════════════════════════════════════════════════════════════════════════
# SAMPLE DATA (for testing without AWS account)
# ═════════════════════════════════════════════════════════════════════════════
...
def sync_sample():
    print(f"[{ts()}] Loading sample AWS inventory data...")
    ...
    for table, rows in data.items():
        ...
        try:
            ...
            duration = int((time.time() - start) * 1000)
            log_sync(sec_conn, table, count, duration)
            print(f"[{ts()}] ✓ {table}: inserted {len(rows)} sample rows")
        except Exception as e:
            print(f"[{ts()}] ✗ {table} failed: {e}")
    ...
    try:
        with conn.cursor() as cur:
            ...
            for tbl, label in [
                ...
                ("aws_security_groups", "Security Groups"),
                ("aws_vpcs",            "VPCs"),
            ]:
                cur.execute(f"SELECT COUNT(*) FROM {tbl}")
                count = cur.fetchone()[0]
                print(f"  {label:<20} {count:>5} rows")

            # Show correlated view counts
            print(f"\n── Correlated views ─────────────────────────────")
            for view, label in [
                ("v_exposed_vulnerable_instances", "Exposed + Vulnerable instances"),
                ("v_public_s3_findings",           "Public S3 with Prowler findings"),
                ("v_risky_iam_users",              "Risky IAM users"),
            ]:
                try:
...
# ═════════════════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Sync AWS inventory via Steampipe into PostgreSQL")
    group  = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--sync",   action="store_true",
                       help="Sync live AWS data via Steampipe (requires AWS credentials)")
    group.add_argument("--sample", action="store_true",
                       help="Load sample data for testing without AWS account")
